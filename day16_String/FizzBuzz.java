...
boolean result = word1.length()==5 && word2.length()==5;
   /*if(result) {
    char ch1 = word1.charAt(word1.length() - 1);//last character first words
...
if(result){
    boolean b1 = word1.endsWith(""+word2.charAt(0));//word ends with first characterof word2
    boolean b2 = word2.startsWith(""+word1.charAt(word1.length()-1)); //word2 starts with last character
//word1
    if (b1 && b2) {
        System.out.println("Fizz");
    }else{
